...
from model_stn_multiloss import STN_MobileNet2
from run_multiloss import train, test, save_checkpoint, find_bounds_clr
...
parser.add_argument('-b', '--batch-size', default=4, type=int, metavar='N', help='mini-batch size (default: 64)')
...
parser.add_argument('--log-interval', type=int, default=5, metavar='N', help='Number of batches between log messages')
...
def main():
    ...
    optimizer = torch.optim.SGD(model.parameters(), args.learning_rate, momentum=args.momentum, weight_decay=args.decay,
                                nesterov=True)
    if args.find_clr:
        find_bounds_clr(model, train_loader, optimizer, PW_criterion, device, dtype, min_lr=args.min_lr,
...
def train_network(start_epoch, epochs, scheduler, model, verified_points, train_loader, val_loader, test_loader, optimizer, criterion,
    ...
    for epoch in trange(start_epoch, epochs + 1):
        if not isinstance(scheduler, CyclicLR):
            scheduler.step()
        train_loss, train_mae, = train(model, verified_points, train_loader, epoch, optimizer, criterion, device,
                                       dtype, batch_size, log_interval, scheduler)
        val_loss, val_mae = test(model, predefined_points, epoch, val_loader, criterion, device, dtype)
        test_loss, test_mae = test(model, predefined_points, epoch, test_loader, criterion, device, dtype)
